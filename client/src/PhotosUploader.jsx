...
export default function PhotosUploader({ addedPhotos, onChange }) {
  ...
  async function addPhotoByLink(ev) {
    ...
    try {
      ...
      // Prepend 'uploads/' when adding a photo by link
  ...
  function uploadPhoto(ev) {
    ...
    axios
      ...
      .then((response) => {
        ...
        // Do not prepend 'uploads/' for direct uploads
      ...
      <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
        {addedPhotos.length > 0 &&
          addedPhotos.map((link, index) => {
            const imageUrl = `http://localhost:4000/${link}`; // Adjust URL based on whether 'uploads/' is already included
            console.log("Image URL:", imageUrl); // Debugging URL construction
            return (
